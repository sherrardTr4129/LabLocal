...
def detect(img):
    ...
    wire = cv2.CascadeClassifier("wireStripper.xml")
    hammerRects = hammer.detectMultiScale(img, 1.3, 4, cv2.cv.CV_HAAR_SCALE_IMAGE, (275,275))
    wireRects = wire.detectMultiScale(img, 1.3, 4, cv2.cv.CV_HAAR_SCALE_IMAGE, (275,275))

    if (len(hammerRects) == 0 and len(wireRects) == 0):
        return [], img, ""
        ...
        return wireRects, img, "wire Stripper"
